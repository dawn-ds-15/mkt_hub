...
export default function ExportData() {
  ...
  const periodMap = { 'Monthly': 'month', 'Quarterly': 'quarter', 'Weekly': 'week', 'Yearly': 'year' };
  ...
  const handleExport = async (type) => {
    ...
    try {
      if (type === 'pdf') {
        ...
        showToast('Full data exported successfully');
      }
      setExported(true);
  ...
  return (
    <div className="space-y-6">
      <div className="mb-2">
        <h2 className="font-headline-lg text-headline-lg text-on-surface">Data Export</h2>
        <p className="text-body-md text-on-surface-variant max-w-2xl">Generate and download performance reports, raw data, and dashboard snapshots for external analysis.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <section className="md:col-span-7 bg-surface-container-lowest border border-outline-variant rounded-xl p-6 flex flex-col">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 flex-1">
            ...
            <div className="space-y-1.5">
              <label className="text-xs text-on-surface-variant font-semibold uppercase tracking-wider">PROJECT</label>
            ...
            <div className="space-y-1.5 sm:col-span-2">
              <label className="text-xs text-on-surface-variant font-semibold uppercase tracking-wider">MEMBER</label>
              <select className="w-full bg-surface-container-low text-body-md border border-outline-variant rounded-lg text-body-md py-2 px-3 focus:ring-primary focus:border-primary transition-all">
        ...
        <section className="md:col-span-5 bg-surface-container-lowest border border-outline-variant rounded-xl p-6 flex flex-col">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <span className="text-xs text-on-surface-variant uppercase tracking-wider">Spreadsheet • XLSX</span>
            </div>
          </div>
          <div className="space-y-4 flex-1">
            <div className="space-y-1.5">
              <label className="text-xs text-on-surface-variant font-semibold uppercase tracking-wider">REPORT PERIOD</label>
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center md:items-start justify-between gap-8">
            <div className="flex-1 text-center md:text-left">
              <div className="flex items-center justify-center md:justify-start gap-3 mb-4">
                ...
                <div>
                  ...
                  <div className="flex items-center justify-center md:justify-start gap-2">
                    <span className="text-xs text-primary uppercase bg-primary-fixed/20 px-2 py-0.5 rounded font-semibold">Full System</span>
                    <span className="flex items-center gap-1 text-xs text-on-surface-variant">
                      <span className="material-symbols-outlined text-xs">verified_user</span> ADMIN ONLY
                    </span>
                  </div>
                </div>
              </div>
              <p className="text-body-md text-on-surface-variant max-w-xl leading-relaxed">
                Export the entire system including workspace data, team interactions, history logs, and custom configuration.
              </p>
              <div className="mt-4 flex flex-wrap justify-center md:justify-start gap-4">
                <div className="flex items-center gap-2 text-xs text-on-surface-variant bg-surface-container px-3 py-1.5 rounded-full border border-outline-variant">
                  <span className="material-symbols-outlined text-[14px]">storage</span> Estimated 450 MB
                </div>
              </div>
            </div>
            <div className="flex flex-col items-center gap-3 w-full md:w-auto">
              ...
              >
                ...
                <span>{exporting ? 'Processing...' : exported ? 'Ready to Download' : 'Export Full Data'}</span>
              </button>
              <p className="text-[10px] text-on-surface-variant opacity-50 text-center uppercase tracking-wider">JSON PACKAGE</p>
            </div>
          </div>
        </section>
        ...
        <div className="md:col-span-12 bg-surface-container-low border border-dashed border-outline-variant rounded-xl p-4 flex items-center justify-center gap-4 flex-wrap">
          ...
          <span className="text-xs text-on-surface-variant ml-2">{exporting ? 'Exporting...' : exported ? 'Exported successfully!' : 'Select a format to export data'}</span>
        </div>
      </div>
    </div>
  );
