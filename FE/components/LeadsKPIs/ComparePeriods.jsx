...
const metricsConfig = [
  ...
  { key: 'CAC / LTV', label: 'CAC', format: (v) => v != null ? formatNum(v) : '\u2014', subField: 'cac' },
  { key: 'CAC / LTV', label: 'LTV', format: (v) => v != null ? formatCurrency(v) : '\u2014', subField: 'ltv' },
];
...
function buildInsights(compareData) {
  ...
  const growthInsight = rl?.percentVsPlan != null && rl?.percentVsPlan > 100
    ? `Raw Leads trong ${YEAR_OPTIONS[0]} vượt kế hoạch ${Math.round(rl.percentVsPlan - 100)}%. Tập trung chuyển đổi giữa phễu (MQL \u2192 SQL) để tối ưu ROI.`
    : `Raw Leads trong ${YEAR_OPTIONS[0]} đạt ${Math.round(rl?.percentVsPlan || 0)}% kế hoạch. Tăng cường chiến dịch tạo lead để đạt mục tiêu.`;
  const sqlWarn = sql?.percentVsPlan != null && sql?.percentVsPlan < 100
    ? `Tỷ lệ SQL đang giảm so với kế hoạch. Xem xét lại tiêu chí đánh giá lead với đội Sales.`
    : `Tỷ lệ SQL đạt theo kế hoạch.`;
  return [
    {
    ...
    {
      title: 'Điểm nổi bật tài chính',
      color: 'border-l-success bg-success/5',
      textColor: 'text-success',
      desc: `Phân tích dựa trên dữ liệu ${YEAR_OPTIONS[0]}. Cập nhật Kế hoạch, Thực tế, Cơ hội và Deal đã đóng để đánh giá chính xác.`,
    },
  ];
}
...
export default function ComparePeriods({ year: currentYear }) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (!compareRes.data || Object.keys(compareRes.data).length === 0) {
        setLoadError('API không trả về dữ liệu so sánh cho kỳ này');
      }
    } catch (error) {
  ...
  const handleExportCSV = () => {
    const headers = ['Chỉ tiêu', ...selectedYears.flatMap(yr => [`Thực tế ${yr}`, `Kế hoạch ${yr}`, `% Kế hoạch`]), ...(selectedYears.length >= 2 ? ['Tăng trưởng YoY'] : [])];
  ...
  const handleDeleteData = async () => {
    if (!window.confirm('Xoá dữ liệu so sánh đã chọn?')) return;
      ...
      <section className="bg-white border border-border-light rounded p-4 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="text-[12px] font-semibold text-on-surface-variant">So sánh năm:</span>
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead>
              <tr className="bg-surface-container-low border-b border-border-light">
                <th className="px-6 py-3 text-[11px] font-bold text-on-surface-variant uppercase tracking-wider sticky left-0 bg-surface-container-low z-10 min-w-[200px]">Chỉ tiêu</th>
                {selectedYears.map((yr, yi) => (
                  <React.Fragment key={yr}>
                    ...
                    {yi === 0 && (
                      <>
                        <th className="px-6 py-3 text-[11px] font-bold text-on-surface-variant uppercase tracking-wider text-right">Kế hoạch {yr}</th>
                        <th className="px-6 py-3 text-[11px] font-bold text-on-surface-variant uppercase tracking-wider text-right">% Kế hoạch</th>
                      </>
                    )}
                  </React.Fragment>
                ))}
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            >
              {['Raw Leads', 'MQL', 'SQL', 'OPP', 'Closed Deal', 'Won Value'].map(m => (
                <option key={m} value={m}>Chỉ tiêu: {m}</option>
              ))}
            </select>
          </div>
        ...
        <div className="bg-white border border-border-light rounded p-4 space-y-4">
          ...
          {aiReport ? (
            ...
            <button onClick={handleGenerateAIReport} disabled={aiLoading} className="w-full py-2.5 bg-surface-container hover:bg-secondary-container transition-colors rounded font-semibold text-secondary flex items-center justify-center gap-2 disabled:opacity-50">
              ...
              {aiLoading ? 'Đang tạo...' : 'Tạo báo cáo AI'}
            </button>
          )}
        </div>
      </div>
